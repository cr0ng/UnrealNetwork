...
ANetProjectile::ANetProjectile()
{
	...
	bReplicates = true;				// 이 액터는 리프리케이션을 한다.
...
void ANetProjectile::BeginPlay()
{
	...
	if (GetInstigator())
	{
		Collision->IgnoreActorWhenMoving(GetInstigator(), true);		// 인스티게이터가 설정되어 있으면 무시하기
	}
}
...
void ANetProjectile::OnHit(AActor* SelfActor, AActor* OtherActor, FVector NormalImpulse, const FHitResult& Hit)
{
	...
	{
		if (!bHitted && OtherActor->IsA<ACharacter>()) // 한번도 충돌한 적 없고, 캐릭터와 캐릭터의 파생클래스만 처리
		{
			// 데미지 주기
			...
			Multicast_HitEffect(Hit.ImpactPoint, Hit.ImpactNormal.Rotation());	// 모두에게 이펙트 재생 보이기
			SetLifeSpan(2.0f);	// 2초뒤에 삭제
			bHitted = true;		// 처음 Hit 되었음을 기록
		}
	}
}
