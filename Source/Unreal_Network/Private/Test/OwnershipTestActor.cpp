...
void AOwnershipTestActor::Tick(float DeltaTime)
{
	...
	DrawDebugSphere(GetWorld(), GetActorLocation(), OverlapCollision->GetScaledSphereRadius(), 32, FColor::Red);
	...
	if (HasAuthority())
	{
		...
		UGameplayStatics::GetAllActorsOfClass(GetWorld(), ANetworkRoleCharacter::StaticClass(), Actors);
		...
		{
			SetOwner(NextOwner);	// 자신에게 가장 가까운 ANetworkRoleCharacter
